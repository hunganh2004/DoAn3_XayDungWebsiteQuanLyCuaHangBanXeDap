...
export default function KhachhangPage() {
    const [list_Khachhang, setList_Khachhang] = useState([])
    ...
    const imageTemplate = (rowData) => {
        if (!rowData || !rowData.anh_khach_hang) {
            return <img  className='product-image' />
        }
        return <img src={rowData.anh_khach_hang}  className='product-image' />;
    }
    const columns = [
        {field: 'id', header: 'ID'},
        {field: 'ten_khach_hang', header: 'Tên khách hàng'},
        {field: 'sdt_khach_hang', header: 'Số điện thoại'},
        {field: 'dia_chi_khach_hang', header: 'Địa chỉ'},
    ...
    const func_delete = (rowData) => {
        if (window.confirm(`Bạn có chắc muốn xóa ${rowData.ten_khach_hang}?`)) {
            khachhangService.deleteByID(rowData.id).then(() => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (formData.id) {
            khachhangService.update([formData]).then(() => {
                alert('Cập nhật thành công!');
                setShowForm(false);
            }).catch(err => console.error('Lỗi khi cập nhật:', err));
        } else {
            khachhangService.addNew([formData]).then(() => {
    ...
    const func_search = (searchValue) => {
        setLoading(true)
        khachhangService.getPaginationSearch(searchValue,pageSize,currentPage).then(d => {
            setList_Khachhang(d)
    ...
    const func_getPagination = () => {
        setLoading(true)
        khachhangService.getPagination(pageSize,currentPage).then(d => {
            setList_Khachhang(d)
            if (d.length < pageSize) {
                setNextPage(false)
            }
    ...
    function fillData() {
        const checkData = [...list_Khachhang]
        while(checkData.length < pageSize) {
            checkData.push('')
        }
        setList_Khachhang(checkData)
    }
    useEffect(() => {
        func_getPagination()
    ...
    return (
        ...
        <div className='table-container'>
            <h1>{loading ? 'Đang tải dữ liệu... ':'Danh sách Khachhang'}</h1>
        ...
        {/* Form thêm/sửa */}
        <Dialog header={formData.id ? 'Chỉnh sửa Khachhang' : 'Thêm Khachhang'} 
        visible={showForm} 
        ...
        <div className='form-container'>
            <form onSubmit={handleSubmit}>
                ...
                <input
                    type='text'
                    value={formData.sdt_khach_hang}
                    onChange={(e) => setFormData({ ...formData, sdt_khach_hang: e.target.value })}
                    required
                />
                <label>Địa chỉ:</label>
                <input
                    type='text'
                    value={formData.dia_chi_khach_hang}
                    onChange={(e) => setFormData({ ...formData, dia_chi_khach_hang: e.target.value })}
                    required
                />
